		...
		match op {
			...
			IndexOperation::Renew { extrinsic, hash } => {
				renewed_map
					.entry(extrinsic)
					.or_default()
					.push(DbHash::from(<[u8; 32]>::try_from(hash.as_ref()).unwrap()));
			},
		}
	}
			...
			match index_map.get(&(index as u32)) {
				Some((hash, size)) => {
					...
					if *size as usize <= encoded.len() {
						...
						transaction.store(
							columns::TRANSACTION,
							DbHash::from(<[u8; 32]>::try_from(hash.as_ref()).unwrap()),
							encoded[offset..].to_vec(),
						);
						DbExtrinsic::Indexed {
							hash: DbHash::from(<[u8; 32]>::try_from(hash.as_ref()).unwrap()),
...
fn apply_indexed_body<Block: BlockT>(transaction: &mut Transaction<DbHash>, body: Vec<Vec<u8>>) {
	for extrinsic in body {
		...
		transaction.store(columns::TRANSACTION, DbHash::from(<[u8; 32]>::try_from(hash.as_ref()).unwrap()), extrinsic);
	}
}
