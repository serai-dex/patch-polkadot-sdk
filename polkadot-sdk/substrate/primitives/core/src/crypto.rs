...
pub fn get_public_from_string_or_panic<TPublic: Public>(
	...
	TPublic::Pair::from_string(&format!("//{}", s), None)
		.expect("Function expects valid argument")
		.public()
}
...
impl alloc::str::FromStr for SecretUri {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		Ok(Self {
			...
			password: cap
				.pass
				.map(|v| SecretString::from_str(v).expect("Returns infallible error")),
...
pub trait Pair: CryptoType + Sized {
	...
	fn from_phrase(
		...
		let big_seed = mnemonic.to_seed_normalized(password.unwrap_or(""));
		let mut seed = Self::Seed::default();
...
/// public modules.
#[derive(
	Copy, Clone, Default, PartialEq, Eq, PartialOrd, Ord, Hash, Encode, Decode, Debug,
)]
